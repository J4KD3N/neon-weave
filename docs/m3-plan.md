...
- **Level cap 12, multiclassing at 5, capstones at 8** per §8, all in `rules/progression` since S31 (D-086). There is no separate demo cap: the demo content cannot bank enough XP to pass level 4.
...
- S31 Progression v2 — done: level cap 12 with tiers at 1/4/7/10 and six new talents; multiclassing from level 5 by moving levels into a second class at the Weave; class capstones at eight levels in one class (six shipped); subclass identities as ability effects (chain, taunt, spreading poison, counter stance, area, party summons with counts); a level-12 8/4 capstone build saves and loads and the Arcanum resets it. D-086.
- S32 Factions v2 — done: `NarrativeState.faction` with `join_faction` as a dialogue or trigger effect (act-gated, exclusive, `join_reputation` through the casting rule), `faction` / `not_faction` conditions, Rootched and Ashfound envoys on the plaza with offer quests, three faction areas off the plaza gated by membership with a vendor each, other envoys locked after an oath, standing shows the oath. D-087.
- S33 Ghost Markets — done: biome palette, five tiles (shutters, arcade tile, failing lights, stalls as cover, echo-static), a `surface_resist` rule (static halves tech damage on whoever stands in it), the Choir family across all five archetypes with traits and an in-family summon (Failed Hollow, Cultist, Echo-wraith, Cantor, Wisp, the Voice as boss), the Broker, Beacon depth 3 finds it, CI renders it, harness rows per biome and depth with a geared party. D-088.
- S34 Null Cathedral — done: fourth biome (nave walls, flagstone, choir lattice, pews as cover, null fonts), a `surface_status` rule (fonts silence whoever starts a turn in them), the cathedral family across all five archetypes with the Organist as boss (Great Hush), story-flag gates on templates (`requires_flag`), and the Loom Approach remix template whose enemy and surface pools are filled by rule from every family and template (`ShardGenerator.expand_remix`). CI renders both; harness rows added. D-089.
- S35 Bastion v2 — done: Archive (a `lore` kind, fragments as pickups in every Shard, story not haul, read in order with gaps, Aether of insight at level 2), Garden (extra heal on return, tending, an Aether per extraction at level 2), Quarters (companion `scenes` gated by approval and the building level, offered from the building, once), seven buildings on the plaza, and `Account` (`user://account.json`) with origins earned by flag across playthroughs (the Beacon Keeper). D-090.
- S36 Campaign tooling v2 — done: quest `branches` (fork on conditions, `next` as fallback, pure forks), one `apply_effects` for triggers, choices and steps, `map_edits` persisted per map in the story and re-applied on entry, `sequence` steps with camera and pauses that hand over to a dialogue or a fight, an `endings` kind resolved by priority and conditions (`party_approval_min` for the Mend) with per-companion epilogues and an ending panel. Phase A complete. D-091.
