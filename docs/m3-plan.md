...
- S36 Campaign tooling v2 — in progress: quest branches landed (a stage's `branches` fork on conditions once its objectives are done, `next` as the fallback, pure fork stages with no objectives). Scripted sequences, world-state edits and the ending state machine follow. D-091 on completion.
