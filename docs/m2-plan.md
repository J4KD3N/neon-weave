...
- S17 Verdant Datacore — done: palette, seven tiles, spore surface (D-070), oval rooms and two-wide corridors, family of four plus mite and the Canopy Matron, Beacon level 2 unlocks it, system menu picks the template, CI renders it. D-069.
- S18 Shard features — done: secrets, Cipher vaults, relay waypoints, merchants, distance ramp, loot rarity; each with a validator rule and saved deltas. Affixes deferred to an item system. D-071.
...
- S25 Steam — done in code: full Platform contract, null backend with a folder cloud, GodotSteam backend via dynamic calls, achievements as content unlocked at story beats, cloud push on save and pull at startup, Steam Input template, steam/README.md. Not verified against a live Steam client: no GodotSteam binaries or app id on this machine. D-078.
...
M2 closed 2026-09-09 with every session merged; unmet exit criteria (external playtesters, art, a live Steam client) carry into the tracks of [docs/m3-plan.md](m3-plan.md) (D-082).
