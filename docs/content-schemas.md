...
| origins | name, dialogue_tag | stat_mods, traits, unlock_flag (earned for the account when a playthrough sets it), unlock_blurb (shown on the creator while the origin is locked, S51) | The same unlock_flag / unlock_blurb pair on a rules/appearance tone or accent makes it a cosmetic the account earns (S62).
