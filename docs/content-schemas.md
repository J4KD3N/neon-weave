...
| maps | name, biome, spawn_marker, legend, rows | npcs (companion / merchant / npc, each with an optional `when`; a companion placement also waits on the flag and never stands in once dead, S38), pickups, enemies, transitions, triggers, doors, buildings | doors[].hack_tech (S63): the protagonist's Tech that opens the gate without its key_flag; the opens_flag is set either way. enemies[].facing (S64: one of e, se, s, sw, w, nw, n, ne; the enemy sees a cone of rules/combat.vision_cone_degrees and only hears within hearing_radius behind it; no facing looks all round), enemies[].sweep and sweep_period (degrees the facing swings each way, seconds per swing). enemies[].patrol [[x,y],...] (S65: cells walked in a loop from the placed cell at patrol_speed cells a second, patrol_wait seconds at each point; the facing follows the walk and a patrol that sees something stops and turns to it; not saved, so a load puts it back at its placed cell).
