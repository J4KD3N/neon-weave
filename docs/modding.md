...
| An ability | `abilities` | `content/abilities/chrome_haymaker.json` | `sound` is an `audio` id; an audio entry is a sidecar with a `file` beside it or a `synth` block (docs/audio-pipeline.md); `effects` carry chain, taunt, poison, counter, aoe, summon_count |
...
| A starting kit | `loadouts` | `content/loadouts/scav_kit.json` | Items worn where they fit the protagonist, the rest in the pack, `resources` banked; `unlock_flag` makes it an account unlock the creator lists as locked until earned |
