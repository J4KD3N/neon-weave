...
- **Pad support is functional, not native** (S53, S69): the stick walks and glides the combat cursor at its tilt, the drawn cursor slides between cells, and rumble marks hits, downs, wins, wipes and extractions; there is no gyro or trackpad path (Steam Input, a Deck in the house), and mouse is still the fastest way to target.
...
- **Saves** (S54). Closed 2026-09-24 (S68, D-124): F5 asks before it overwrites and F9 is one confirm from slot 1; older autosaves are a setting (one to five); the autosave carries a fight in progress (the window's close button writes it mid-fight, saves are v3), so an Iron Weave run walked out of resumes where it stood. Left: titles are made, not typed; thumbnails need a rendered frame; content mismatches are refused (D-080); a save inside a Shard is refused when `ShardGenerator.LAYOUT_VERSION` moves, which is by hand; a crash mid-fight still falls back to the pre-fight checkpoint.

- **Movement**. Closed 2026-09-24 (S69, D-125): the leader cannot step into an enemy or an NPC and a click path goes round them; followers hold rather than crowd anyone ahead or walk into anyone; the party steps into its combat cells over a beat when the fight animates; the pad cursor glides between cells. Left: WASD can still stand you visually behind a tall wall (a y-sort question for the art track).
