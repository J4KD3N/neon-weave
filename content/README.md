...
| `rules` | combat, attributes | `combat`: tunables read by `CombatRules`. `attributes`: creator point buy (`names`, `points`, `max_per_attribute`, per-point `effects`). `appearance`: creator tones and accents (`{id, name, color}` lists, S51). |
| `difficulties` | story, balanced, tactician | Title-screen difficulties (S50): `summary`, `order`, `default`, and `rules`, an overlay `{rules entry id: {field: value}}` written over that entry for the story. |
| `locales` | en, pseudo | Languages (S55): `ui` {English source: text}, `content` {kind.id.path: text}; `pseudo` is the test transform. See docs/localization.md. |
