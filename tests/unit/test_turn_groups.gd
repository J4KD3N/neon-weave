...
func test_move_undo_until_anything_else_happens() -> void:
	var s := _state(_four())
	var a := s.by_id("a")
	assert_false(s.can_undo_move(a), "nothing to undo yet")
	assert_false(s.undo_move())
	assert_true(s.move(a, Vector2i(0, 1)))
	assert_eq(a.move_left, 5)
	assert_true(s.can_undo_move(a))
	assert_true(s.undo_move())
	assert_eq(a.cell, Vector2i(0, 0))
	assert_eq(a.move_left, 6, "Move refunded")
	assert_false(s.can_undo_move(a), "one undo per move")
	assert_any_contains(s.history, "a steps back to (0, 0)")
	assert_true(s.move(a, Vector2i(1, 1)))
	assert_true(s.use_ability(a, "sure", Vector2i(2, 0)).has("hit"))
	assert_false(s.can_undo_move(a), "an ability commits the move")
	assert_true(s.move(a, Vector2i(0, 1)))
	assert_eq(s.switch_to("b"), "")
	assert_false(s.can_undo_move(s.by_id("b")), "a swap commits it too")
	assert_eq(s.switch_to("a"), "")
	assert_false(s.can_undo_move(a))
	var b := s.by_id("b")
	s.switch_to("b")
	assert_true(s.move(b, Vector2i(1, 1)))
	s.end_turn()
	assert_eq(s.current(), a)
	assert_false(s.can_undo_move(a), "ending a turn commits the mover")
	assert_false(s.undo_move())
