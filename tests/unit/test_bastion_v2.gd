...
func test_a_quarters_scene_fires_from_approval_once() -> void:
	...
	assert_true(world.roster_menu.visible, "the Quarters are the Roster screen since S52")
	assert_contains(world.roster_menu.label.text, "Sera — walks with you")
	assert_contains(world.roster_menu.label.text, "↳ ")
	world.close_roster()
