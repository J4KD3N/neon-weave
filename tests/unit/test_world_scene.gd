...
func test_subclass_and_talents_persist_through_save_and_load() -> void:
	...
	assert_eq(world.buy_talent("weaver", "iron_skin"), "needs 1 Aether", "a Trueborn pays one less (D-085)")
	world.ledger.bank({"aether": 5})
	assert_eq(world.buy_talent("weaver", "iron_skin"), "")
	assert_eq(world.ledger.total("aether"), 4)
...
func test_depth_three_is_measurably_harder_and_posts_a_boss() -> void:
	...
	assert_eq(boss.max_hp, int(round(int(world.registry.get_entry("enemies", "undercity_warlord")["stats"]["hp"]) * world.rules.boss_hp_mult * (1.0 + world.rules.depth_hp_per_level * 2.0))), "boss HP at depth 3 follows the rules")
