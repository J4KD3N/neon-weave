...
func test_silence_blocks_arcane_for_its_duration() -> void:
	...
	s.end_turn()
	assert_eq(s.current(), mage)
	assert_eq(int(mage.statuses["silenced"]), 2, "two of the target's own turns")
	...
	s.end_turn()
	assert_eq(s.current(), mage)
	assert_eq(int(mage.statuses["silenced"]), 1, "counted down when its turn ended")
	assert_eq(s.can_use(mage, "bolt", blade.cell), "silenced")
	s.end_turn()
	s.end_turn()
	assert_eq(s.current(), mage)
	assert_false(mage.is_silenced(), "expired after two silenced turns")
