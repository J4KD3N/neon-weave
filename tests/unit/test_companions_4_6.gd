...
func test_six_recruited_means_a_bench_and_the_roster_at_home() -> void:
	...
	assert_true(world.roster_menu.visible, "the Roster is its own screen since S52")
	assert_contains(world.roster_menu.label.text, "Whisper — waits at the Bastion")
	assert_contains(world.roster_menu.label.text, "Dax — walks with you")
