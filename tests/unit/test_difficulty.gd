...
func test_iron_weave_is_mortal_with_one_save_and_no_reloads() -> void:
	...
	assert_false(bool(_row(items, "save_1")["enabled"]))
	assert_eq(String(_row(items, "save_1")["why"]), "Iron Weave: one save")
	assert_false(bool(_row(items, "load_autosave")["enabled"]))
	assert_eq(String(_row(items, "load_autosave")["why"]), "Iron Weave: no reloads")
	assert_false(bool(_row(items, "load_1")["enabled"]))
