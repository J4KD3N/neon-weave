	...
	for m: Dictionary in world.registry.get_all("maps"):
		...
		for tr: Dictionary in m.get("triggers", []):
			...
			for k: String in tr.get("when", {}):
				assert_true(["flags", "origin_tag", "race", "race_tag", "class", "approval", "reputation", "recruited", "not_recruited", "quest", "faction", "not_faction", "romance", "not_romance", "romance_open", "party_approval_min", "party_race", "party_race_tag", "attribute", "disguised", "walking", "benched", "map"].has(k), "map %s trigger when.%s" % [m["id"], k])
