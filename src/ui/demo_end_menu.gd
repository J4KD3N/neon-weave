...
static func render(stats: Dictionary) -> String:
	...
	lines.append(Loc.t("THE WAKING — END OF THE DEMO"))
	lines.append("")
	lines.append(Loc.t("It knew your name. The Waking goes on from here: the second and third depths, the Verdant Datacore, the Loom, and the choice the Sundering never finished making."))
	lines.append("")
	...
	lines.append(Loc.t("Walking with you: %s") % (", ".join(PackedStringArray(companions)) if not companions.is_empty() else Loc.t("nobody, which is its own kind of story")))
	lines.append(String(stats.get("standing", "")))
	if stats.has("choice"):
		lines.append(String(stats["choice"]))
	lines.append("")
	lines.append(Loc.t("Thank you for playing. Neon Weave is open source: the whole demo is data you can read and change."))
	lines.append("")
	lines.append(Loc.t("Enter / A / Esc: back to the Bastion"))
	return "\n".join(lines)
